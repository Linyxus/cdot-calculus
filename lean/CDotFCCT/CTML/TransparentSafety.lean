import CDotFCCT.CTML.TransparentCalculus

/-!
# Safety of the experimental record-inversion calculus

The fundamental lemma covers native records, both subtyping observations, arbitrary
constraint abstraction, impredicative universals, Z, and function-guarded local
recursive definitions and simultaneous recursive function equations. The evaluator
and reduction rules are unchanged.
-/
...
mutual
  ...
  theorem nativeFields {s : SubtypingContext} {context : TypingContext s.typeDepth}
      ...
      FieldsTyping s context fields types :=
    match typing with
    | .nil => .nil
...
mutual
  theorem HasType.sound {s : SubtypingContext} {context : TypingContext s.typeDepth}
      {term : Term} {type : WFTy s.typeDepth} (typing : HasType s context term type) :
      Typing s context term type :=
    match typing with
  ...
  theorem FieldsHaveType.sound {s : SubtypingContext}
      ...
      {types : List (FieldName × WFTy s.typeDepth)}
      (typing : FieldsHaveType s context fields types) :
      FieldsTyping s context fields types :=
    match typing with
    | .nil => .nil
...
theorem Typing.closed {term : Term} {type : WFTy 0}
    (typing : Typing SubtypingContext.empty TypingContext.empty term type)
    (env : Indexed.Environment) (n : Nat) :
    Indexed.Computation (fun k => (interpret env type.raw k).1) n term :=
  Indexed.instantiate_empty term ▸ typing env n Term.var
    (empty_validates env n) (Valuation.empty env n Term.var)
...
theorem HasType.safe {term reached : Term} {type : WFTy 0}
    (typing : HasType SubtypingContext.empty TypingContext.empty term type)
    (steps : Steps term reached) : Value reached ∨ ∃ next, Step reached next :=
