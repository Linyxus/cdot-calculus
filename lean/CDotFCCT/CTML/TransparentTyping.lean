import CDotFCCT.CTML.TransparentInversion
import CDotFCCT.CTML.TransparentValuation
import CDotFCCT.CTML.TransparentRuntime
import CTMLCore.Declarative.IndexedFixpoint

/-! # Semantic typing with transparent records and function-guarded recursion -/
...
def Typing (s : SubtypingContext) (context : TypingContext s.typeDepth)
    (term : Term) (type : WFTy s.typeDepth) : Prop :=
  ∀ env n substitute, Validates s env n → Valuation env context n substitute →
    Computation (fun k => (interpret env type.raw k).1) n
      (instantiate s.typeDepth substitute term)

def FieldsTyping (s : SubtypingContext) (context : TypingContext s.typeDepth)
    {names : List FieldName} (fields : TermFields names)
    (types : List (FieldName × WFTy s.typeDepth)) : Prop :=
  ∀ env n substitute, Validates s env n → Valuation env context n substitute →
    FieldsComputation env n (instantiateFields s.typeDepth substitute fields) types
...
theorem Typing.var {s : SubtypingContext} {context : TypingContext s.typeDepth}
    ...
    Typing s context (.var index) type :=
...
theorem Typing.subsumption {s : SubtypingContext} {context : TypingContext s.typeDepth}
    {term : Term} {sub sup : WFTy s.typeDepth} (typing : Typing s context term sub)
    (subtype : InvertingSubtype s sub sup) : Typing s context term sup :=
...
theorem Typing.forall {s : SubtypingContext} {context : TypingContext s.typeDepth}
    ...
    (typing : Typing s.bindType context.bindType (term.liftTy 1) body) :
    Typing s context term (WFTy.all body) := by
  intro env n substitute valid valuation
  refine .ofValue (nonexpansive_instantiate valuation nonexpansive inScope) ?_
  exact fun argument =>
    (show Computation (fun k => (interpret (env.cons argument) body.raw k).1) n
...
theorem Typing.constrained {s : SubtypingContext} {context : TypingContext s.typeDepth}
    ...
    (inScope : ∀ index, term = .var index → index < context.entries.length)
    (typing : Typing (s.assume guard) context term body) :
    Typing s context term (WFTy.constrained guard body) :=
...
theorem Typing.recursive {s : SubtypingContext} {context : TypingContext s.typeDepth}
    {term : Term} {type : WFTy s.typeDepth} (definition : Definition s.typeDepth)
    (typing : Typing (definition.native.openContext s) context.bindType
  ...
  simpa only [interpret_lift, instantiate_liftTy] using
    (show Computation (fun k =>
      (interpret (env.cons (definition.interpretation env)) (type.raw.lift 1) k).1) n
...
theorem Typing.abstraction {s : SubtypingContext} {context : TypingContext s.typeDepth}
    ...
    (typing : Typing s (context.bind param) body ret) :
    Typing s context (.abs body) (WFTy.arrow param ret) :=
...
theorem Typing.application {s : SubtypingContext} {context : TypingContext s.typeDepth}
    ...
    (functionTyping : Typing s context function (WFTy.arrow param ret))
    (argumentTyping : Typing s context argument param) :
    Typing s context (.app function argument) ret :=
  fun env n substitute valid valuation =>
    (instantiate_app s.typeDepth substitute function argument).symm ▸
      Computation.application (interpret_downward param.raw env)
...
theorem Typing.record {s : SubtypingContext} {context : TypingContext s.typeDepth}
    {names : List FieldName} {fields : TermFields names}
    {types : List (FieldName × WFTy s.typeDepth)} (typing : FieldsTyping s context fields types)
    (className : ClassName) : Typing s context (.record className fields)
...
theorem Typing.projection {s : SubtypingContext} {context : TypingContext s.typeDepth}
    ...
    (typing : Typing s context term (WFTy.record field type)) :
    Typing s context (.proj term field) type :=
  fun env n substitute valid valuation =>
    (instantiate_proj s.typeDepth substitute term field).symm ▸
      Transparent.projection (interpret_downward type.raw env)
        (typing env n substitute valid valuation)

theorem Typing.ascription {s : SubtypingContext} {context : TypingContext s.typeDepth}
    {term : Term} {sub sup : WFTy s.typeDepth} (typing : Typing s context term sub)
    (subtype : InvertingSubtype s sub sup) : Typing s context (.ascribe term sup.raw) sup :=
  fun env n substitute valid valuation =>
    (instantiate_ascribe s.typeDepth substitute term sup.raw).symm ▸
      Computation.ascription (interpret_downward sup.raw env)
...
theorem Typing.intersection {s : SubtypingContext} {context : TypingContext s.typeDepth}
    {term : Term} {left right : WFTy s.typeDepth} (first : Typing s context term left)
    (second : Typing s context term right) : Typing s context term (WFTy.intersection left right) :=
...
theorem Typing.ifThen {s : SubtypingContext} {context : TypingContext s.typeDepth}
    ...
    (typing : Typing s context scrutinee scrutineeType)
    (subtype : InvertingSubtype s scrutineeType (WFTy.cls className))
    (branch : Typing s (context.bind scrutineeType) yes result) :
    Typing s context (.ifIs scrutinee className yes no) result :=
  fun env n substitute valid valuation =>
    (instantiate_ifIs s.typeDepth substitute scrutinee className yes no).symm ▸
      Computation.ifThen (interpret_downward scrutineeType.raw env)
...
theorem Typing.ifElse {s : SubtypingContext} {context : TypingContext s.typeDepth}
    {scrutinee yes no : Term} {className : ClassName} {scrutineeType result : WFTy s.typeDepth}
    (typing : Typing s context scrutinee scrutineeType)
    (subtype : InvertingSubtype s scrutineeType (WFTy.neg (WFTy.cls className)))
    (branch : Typing s (context.bind scrutineeType) no result) :
    Typing s context (.ifIs scrutinee className yes no) result :=
  fun env n substitute valid valuation =>
    (instantiate_ifIs s.typeDepth substitute scrutinee className yes no).symm ▸
      Computation.ifElse (interpret_downward scrutineeType.raw env)
        (typing env n substitute valid valuation) className _ _
        (fun k within term => (subtype.sound env n valid k within term).1)
        (fun k within term value related =>
...
theorem FieldsTyping.cons {s : SubtypingContext} {context : TypingContext s.typeDepth}
    ...
    {types : List (FieldName × WFTy s.typeDepth)} (head : Typing s context value type)
    (rest : FieldsTyping s context tail types) :
    FieldsTyping s context (.cons name value tail fresh) ((name, type) :: types) :=
  fun env n substitute valid valuation =>
